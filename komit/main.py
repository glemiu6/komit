...
from komit.generator import STYLES, generate_message
...
from komit.update_utils import check_for_updates, uninstall, update
...
def parse_args(argv=None):
    ...
    config_group.add_argument(
        "--config", type=str, metavar="<path>", help="Path to custom configuration TOML file"
    )
    ...
    execution_group.add_argument(
        "--version", action="version", version=f"{__version__}", help="Version of the package"
    )
    # flag to test the commits without executing
    execution_group.add_argument(
        "--dry-run", "-dr", action="store_true", help="Don't actually commit the changes."
    )
    # flag for explaining the changes without commiting
    execution_group.add_argument(
        "--explain", action="store_true", help="Explain staged changes without committing."
    )
    execution_group.add_argument(
        "--deep",
        action="store_true",
...
def run():
    ...
    console.print(
        f"Model: [cyan]{config.model}[/cyan] · Style: [cyan]{config.style}[/cyan]", style="dim"
    )  # noqa: E501
    if args.explain:
    ...
    try:
        status_msg = (
            "Generating commit message (deep mode)..."
            if args.deep
            else "Generate commit message..."
        )  # noqa: E501
        with console.status(status_msg, spinner="dots"):
            message, truncated = generate_message(
                diff=diff, config=config, branch_info=branch_name, deep=args.deep
            )  # noqa: E501
            if truncated:
                console.print(
                    f"[yellow]!! Large files were truncated: {', '.join(truncated)}[/yellow]"
                )  # noqa: E501
            ...
            case "r":
                try:
                    with console.status("Regenerating...", spinner="dots"):
                        message, truncated = generate_message(
                            diff=diff, config=config, branch_info=branch_name
                        )  # noqa: E501
                    console.print(
                        Panel(message, title="New suggested message:", border_style="green")
                    )  # noqa: E501
                except RuntimeError as e:
